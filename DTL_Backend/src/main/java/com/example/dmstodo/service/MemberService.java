...
public class MemberService {
    ...
    public MemberResDto signup(MemberSignUpDto req) {
        ...
        memberRepository.save(Member.builder()
                ...
                .userRoles(req.getUserId().equals("admin") ? Collections.singletonList("ROLE_ADMIN") :  Collections.singletonList("ROLE_USER"))
                .build());
    ...
    public MemberResDto login(MemberSignInDto req) {
        ...
        String token = jwtTokenProvider.createToken(member.getUserId(), member.getUserRoles());
        return MemberResDto.builder()
                .message("로그인 성공")
                .name(token)
                .build();
